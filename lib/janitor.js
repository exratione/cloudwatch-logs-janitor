 ...
 * @param {RegExp} [options.exclude] Exclude log groups with matching names.
...
Janitor.prototype.getMatchingLogGroups = function (options, callback) {
  ...
  if (options.exclude && !_.isRegExp(options.exclude)) {
    return callback(new Error(
      'If provided, options.exclude must be a RegExp instance.'
    ));
  }
  ...
  function describeLogGroupsRecusively (token) {
    self.cwlClient.describeLogGroups({
      ...
      // Winnow log groups with the filters before appending to the list.
      logGroups = logGroups.concat(_.filter(
        results.logGroups,
        function (logGroup) {
          var keep = logGroup.creationTime < options.createdBefore;

          if (options.exclude) {
            keep = keep && !options.exclude.test(logGroup.logGroupName);
          }
